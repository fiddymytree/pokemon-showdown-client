...
class BattleTooltips {
	...
	calculateModifiedStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon, statStagesOnly?: boolean) {
		...
		if (clientPokemon) {
			...
			for (const statName of Dex.statNamesExceptHP) {
				if (clientPokemon.volatiles['protosynthesis' + statName] || clientPokemon.volatiles['quarkdrive' + statName] || clientPokemon.volatiles['neurodrive' + statName] || clientPokemon.volatiles['protomorphosis' + statName]) {
					if (statName === 'spe') {
		...
		if (this.battle.hasPseudoWeather('Psychic Terrain')) {
			if (ability === 'neuroengine') {
				stats.spa = Math.floor(stats.spa * 1.1111);
			}
		}
		if (this.battle.hasPseudoWeather('Electric Terrain')) {
